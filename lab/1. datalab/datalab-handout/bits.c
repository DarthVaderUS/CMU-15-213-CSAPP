...
int allOddBits(int x) {
    /* Create mask 0xAAAAAAAA (1010... in binary), AND with x, and check if equal to mask */
...
int isAsciiDigit(int x) {
    /* Check if x - 0x30 <= 0x09 by computing difference and checking sign */
    int index=x+(~0x30+1);
    int upper_bound=index+(~0x09+1);
    int sign=upper_bound>>31;
    return !sign&!(x>>8);
}
/* 
...
int howManyBits(int x) {
    /* Use binary search to find the highest bit set, account for sign bit */
  int sign = x >> 31;
  x = (sign & ~x) | (~sign & x); // Get absolute value
  ...
  b0 = x;
  result += b0;
  return result + 1; // Add 1 for sign bit
}
//float
/* 
